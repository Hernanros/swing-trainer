...
def get_stats(
    ...
    def _stats(trades):
        ...
        scores = [t.checklist_score for t in trades if t.checklist_score is not None]
        return {
    ...
    real   = [t for t in all_trades if not t.practice]
    paper  = [t for t in all_trades if t.practice]
    result = _stats(real)
    result["paper"] = _stats(paper)
    return result
